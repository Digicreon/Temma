...
class Post extends \Temma\Web\Attribute {
	 ...
	 * @param	?string		$flashVar		(optional) Name of the session flash variable which will contain the invalid POST data in case of redirection. ("form" by default)
	 * @param	?string		$dataVar		(optional) Name of the template variable which will contain the validated POST data.
	 */
	...
	public function apply(\Reflector $context) : void {
		try {
			if (!$this->dataVar) {
				$this->_request->validateInput($this->contract, 'POST', $this->strict);
			} else {
				$data = null;
				$this->_request->validateInput($this->contract, 'POST', $this->strict, $data);
