 ...
 */
class CachePlugin extends \Temma\Web\Plugin {
	/**
	...
	public function preplugin() {
		TµLog::log('Temma/Web', 'INFO', "TemmaCachePlugin started.");
		...
		$dataSource = $cacheConf['source'];
		if (empty($dataSource)) {
			TµLog::log('Temma/Web', 'DEBUG', "No cache configuration.");
			return (self::EXEC_FORWARD);
		}
		$cache = $this->_loader->dataSources[$dataSource];
		if (!isset($cache)) {
		...
		// URL check
		$isCacheable = false;
		if (isset($cacheConf['url']) && is_array($cacheConf['url']) &&
